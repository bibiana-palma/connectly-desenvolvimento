...
function BudgetDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setCustomStatusIds((assignmentsData || []).map((a: any) => a.status_id));
  ...
  const handleSave = async () => {
    ...
    // Replace status assignments
    await supabase.from("budget_status_assignments").delete().eq("budget_id", id);
    if (customStatusIds.length) {
      const { error: eAssign } = await supabase.from("budget_status_assignments").insert(
        customStatusIds.map((sid) => ({ budget_id: id, status_id: sid, user_id: user.id })),
      );
      if (eAssign) {
        toast.error(eAssign.message);
        setSaving(false);
        return;
      }
    }
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        <h1 className="font-display italic text-2xl xl:text-3xl text-primary">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3 print:hidden">
        <button
          ...
          className="px-4 py-2.5 rounded-full bg-destructive text-destructive-foreground text-sm font-semibold flex items-center gap-2 justify-center"
        >
